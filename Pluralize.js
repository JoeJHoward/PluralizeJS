...
var Pluralize = function(word, count)
{
    ...
    /**
     * The count to convert to from
     *
     * @var number
     */
    this.count = (typeof count === 'undefined' ? 2 : count);
 ...
 * @return string
 */
Pluralize.prototype.convert = function()
{
    ...
    this.lowercase = strtolower(this.word);
    this.upperCase = strtoupper(this.word);
    this.sentenceCase = ucfirst(this.word);
    ...
    const irregular = this.isIrregular();
    ...
    if (this.sibilants.includes(this.suffix(this.lowercase, 1)) || (this.suffix(this.lowercase, 2) === 'ch'))
    {
        return this.toCasing(this.word + 'es', this.casing);
    }

    // Nouns that end in a vowel + y take the letter s:
    if (this.vowels.includes(this.nthLast(this.lowercase, 1)) && this.suffix(this.lowercase, 1) === 'y')
    {
        return this.toCasing(this.word + 's', this.casing);
    }

    // Nouns that end in a consonant + y drop the y and take ies:
    if (this.consonants.includes(this.nthLast(this.lowercase, 1)) && this.suffix(this.lowercase, 1) === 'y')
    {
        return this.toCasing(this.sliceFromEnd(this.word, 1) + 'ies', this.casing);
    }

    // Nouns that end in a consonant + o add s:
    if (this.consonants.includes(this.nthLast(this.lowercase, 1)) && this.suffix(this.lowercase, 1) === 'o')
    {
        return this.toCasing(this.word + 's', this.casing);
    }

    // Nouns that end in a vowel + o take the letter s:
    if (this.vowels.includes(this.nthLast(this.lowercase, 1)) && this.suffix(this.lowercase, 1) === 'o')
    {
        return this.toCasing(this.word + 's', this.casing);
    }
    ...
    const pluralized = this.autoSuffix();

    if (pluralized)
    {
        return this.toCasing(this.sliceFromEnd(this.word, pluralized[0]) + pluralized[1], this.casing);
    }
...
Pluralize.prototype.isUncountable = function()
{
    ...
    return uncountable.includes(this.lowercase);
}
 ...
 *
 * @param  {String} word  The word to convert
 * @param  {Number} count The index to get
 * @return string
 */
export function pluralize(text, count)
{
    return (new Pluralize(text, count)).convert();
}
